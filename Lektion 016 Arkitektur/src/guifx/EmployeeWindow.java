...
public class EmployeeWindow extends Stage {
	...
	private void initControls() {
		if (employee != null) {
			...
			txfYear.setText("" + employee.getEmpYear());
	...
	private void okAction() {
		...
		int empYear = 0;
		empYear = Integer.parseInt(txfYear.getText().trim());
		// skal noget addes her?
		...
		if (employee != null) {
			Controller.updateEmployee(employee, name, wage, empYear);
			...
			if (companyIsSelected) {
				Controller.createEmployee(name, wage, empYear, newCompany);
			} else {
				Controller.createEmployee(name, wage, empYear);
			}
		}
		hide();
	}
